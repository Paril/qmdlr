...
void Camera::lookAt(const Vector3 &eye, const Vector3 &target, const Vector3 &up)
{
    ...
    m_xAxis = Vector3::crossProduct(up, m_zAxis);
    m_xAxis.normalize();

    m_yAxis = Vector3::crossProduct(m_zAxis, m_xAxis);
    m_yAxis.normalize();
    m_xAxis.normalize();
    ...
    m_viewMatrix(0, 3) = -Vector3::dotProduct(m_xAxis, eye);

    m_viewMatrix(1, 0) = m_yAxis.x();
    m_viewMatrix(1, 1) = m_yAxis.y();
    m_viewMatrix(1, 2) = m_yAxis.z();
    m_viewMatrix(1, 3) = -Vector3::dotProduct(m_yAxis, eye);

    m_viewMatrix(2, 0) = m_zAxis.x();
    m_viewMatrix(2, 1) = m_zAxis.y();
    m_viewMatrix(2, 2) = m_zAxis.z();    
    m_viewMatrix(2, 3) = -Vector3::dotProduct(m_zAxis, eye);

    // Extract the pitch angle from the view matrix.
    m_accumPitchDegrees = qRadiansToDegrees(asinf(m_viewMatrix(2, 1)));
    
    m_orientation = m_orientation.fromRotationMatrix(m_viewMatrix.toGenericMatrix<3, 3>());
}
...
void Camera::move(float dx, float dy, float dz)
{
    ...
    if (m_behavior == CameraBehavior::CAMERA_BEHAVIOR_FIRST_PERSON)
    {
        ...
        forwards = Vector3::crossProduct(WORLD_YAXIS, m_xAxis);
        forwards.normalize();
    }
    else
    {
...
void Camera::perspective(float fovx, float aspect, float znear, float zfar)
{
    ...
    float e = 1.0f / tanf(qDegreesToRadians(fovx) / 2.0f);
...
void Camera::updatePosition(const Vector3 &direction, float elapsedTimeSec)
{
    ...
    if (m_currentVelocity.lengthSquared() != 0.0f)
    {
        ...
        if (direction.x() == 0.0f && qFuzzyCompare(m_currentVelocity.x(), 0.0f))
            displacement[0] = 0.0f;

        if (direction.y() == 0.0f && qFuzzyCompare(m_currentVelocity.y(), 0.0f))
            displacement[1] = 0.0f;

        if (direction.z() == 0.0f && qFuzzyCompare(m_currentVelocity.z(), 0.0f))
            displacement[2] = 0.0f;
...
{
    Matrix4 m(newOrientation.toRotationMatrix());
    ...
    m_accumPitchDegrees = qRadiansToDegrees(asinf(m(1, 2)));
...
void Camera::rotateFirstPerson(float headingDegrees, float pitchDegrees)
{
    ...
    {
        rot = rot.fromAxisAndAngle(WORLD_YAXIS, headingDegrees);
    ...
    {
        rot = rot.fromAxisAndAngle(WORLD_XAXIS, pitchDegrees);
...
void Camera::rotateFlight(float headingDegrees, float pitchDegrees, float rollDegrees)
{
    ...
    rot = rot.fromEulerAngles(pitchDegrees, headingDegrees, rollDegrees);
    m_orientation *= rot;
}
...
void Camera::rotateOrbit(float headingDegrees, float pitchDegrees, float rollDegrees)
{
    ...
    if (m_preferTargetYAxisOrbiting)
    {
        if (headingDegrees != 0.0f)
        {
            rot = rot.fromAxisAndAngle(WORLD_XAXIS, headingDegrees);
            m_orientation = rot * m_orientation;
        ...
        if (pitchDegrees != 0.0f)
        {
            rot = rot.fromAxisAndAngle(m_targetYAxis, pitchDegrees);
            m_orientation = m_orientation * rot;
            m_orientation.normalize();
        }
    }
...
void Camera::updateViewMatrix()
{
    ...
    m_viewMatrix = Matrix4(m_orientation.toRotationMatrix());

    m_xAxis = { m_viewMatrix(0, 0), m_viewMatrix(0, 1), m_viewMatrix(0, 2) };
    m_yAxis = { m_viewMatrix(1, 0), m_viewMatrix(1, 1), m_viewMatrix(1, 2) };
    m_zAxis = { m_viewMatrix(2, 0), m_viewMatrix(2, 1), m_viewMatrix(2, 2) };
    m_viewDir = -m_zAxis;
    ...
    m_viewMatrix(0, 3) = -Vector3::dotProduct(m_xAxis, m_eye);
    m_viewMatrix(1, 3) = -Vector3::dotProduct(m_yAxis, m_eye);
    m_viewMatrix(2, 3) = -Vector3::dotProduct(m_zAxis, m_eye);
}